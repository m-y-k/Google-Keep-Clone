import { useContext } from "react";
import "../notes/note.css";

import { Card, CardContent, CardActions, Typography } from "@mui/material";
import { styled } from "@mui/material/styles";
import { CloseRounded as Close, UndoRounded as Restore } from "@mui/icons-material";
import { DataContext } from "../../context/DataProvider";

const StyledCard = styled(Card)`
  border: 1px solid #e0e0e0;
  border-radius: 8px;
  width: 240px;
  margin: 8px;
  box-shadow: none;
`;

const Options = styled(CardActions)`
    cursor: pointer;
`;

const DeleteNote = ({ deleteNote }) => {
  const { deleteNotes, setNotes, setAcrchiveNotes, setDeleteNotes, setSearchNotes } =
    useContext(DataContext);

  const restoreNote = (deleteNote) => {
    const updatedNotes = deleteNotes.filter(
      (data) => data.id !== deleteNote.id
    );
    setDeleteNotes(updatedNotes);
    setNotes((prevArr) => [deleteNote, ...prevArr]);
    // setSearchNotes((prevArr) => [deleteNote, ...prevArr]);
  };

  const removeNote = (deleteNote) => {
    const updatedNotes = deleteNotes.filter(
      (data) => data.id !== deleteNote.id
    );
    setDeleteNotes(updatedNotes);
  };

  return (
    <StyledCard className="card-item" style={{backgroundColor: deleteNote.color}}>
      <CardContent>
        <Typography>{deleteNote.heading}</Typography>
        <Typography>{deleteNote.text}</Typography>
      </CardContent>
      <Options>
        <Close
          fontSize="medium"
          style={{ marginLeft: "auto" }}
          onClick={() => removeNote(deleteNote)}
        />
        <Restore fontSize="medium" onClick={() => restoreNote(deleteNote)} />
      </Options>
    </StyledCard>
  );
};

export default DeleteNote;
